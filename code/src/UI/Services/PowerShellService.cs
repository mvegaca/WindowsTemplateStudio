...
namespace Microsoft.Templates.UI.Services
{
    public partial class PowerShellService
    {
        ...
        public bool IsExecutionPolicyRestricted()
        {
            ...
            var result = RunWithProcess(settings);
            return result.Result.Contains(_restrictedExecutionPolicy);
        }

        public void SetExecutionPolicyUnrestricted()
        {
            var settings = new PSCommandSettings() { FileName = "SetExecutionPolicyUnrestricted", Command = PsCommand.Common };
            var result = RunWithProcess(settings);
        }
        ...
        public void ConnectAzureAD()
        {
            var settings = new PSCommandSettings() { FileName = "ConnectAzureAD", Command = PsCommand.Aad };
            var result = RunWithProcess(settings);
        }
        ...
        private PSResult RunWithProcess(PSCommandSettings commandSettings)
        {
            try
            {
                var result = new PSResult();
                var process = Process.Start(commandSettings.GetProcessInfo());
